...
void svt_aom_generate_r0beta(PictureParentControlSet *pcs) {
    ...
    for (uint32_t sb_y = 0; sb_y < picture_sb_height; ++sb_y) {
        for (uint32_t sb_x = 0; sb_x < picture_sb_width; ++sb_x) {
            ...
            if (scs->static_config.balancing_q_bias) {
                mc_dep_delta_sum = CLIP3(recrf_dist_sum << 9, recrf_dist_sum << 12, mc_dep_delta_sum);
...
static void aom_av1_set_mb_ssim_rdmult_scaling(PictureParentControlSet *pcs) {
    ...
    for (int row = 0; row < num_rows; ++row) {
        for (int col = 0; col < num_cols; ++col) {
            ...
            if (pcs->scs->static_config.tune == 3 || pcs->scs->static_config.alt_ssim_tuning) {
                ...
                for (int mi_row_3 = mi_row; mi_row_3 < cm->mi_rows && mi_row_3 < (row + 1) * num_mi_h; mi_row_3 += 1) {
                    for (int mi_col_3 = mi_col; mi_col_3 < cm->mi_cols && mi_col_3 < (col + 1) * num_mi_w; mi_col_3 += 1) {
                        ...
                        else if (pcs->scs->static_config.tune == 3)
